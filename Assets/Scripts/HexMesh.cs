...
public class HexMesh : MonoBehaviour
{
    ...
    void CreateChunks()
    {
        ...
        for (int z = 0, i = 0; z < HexMetrics.chunkCountZ; z++)
        {
            for (int x = 0; x < HexMetrics.chunkCountX; x++)
            {
                ...
                chunk.GetComponent<HexMeshChunk>().Init(x, z);
            }
        }
    }
    ...
    void CreateBuilding()
    {
        for (int z = 0; z < ProvinceRepository.Instance.Height; z++)
        {
            for (int x = 0; x < ProvinceRepository.Instance.Width; x++)
            {
                Province province = ProvinceRepository.Instance.GetProvince(x, z);
                ...
                else if(province.ProvinceType == ProvinceType.Road)
                {
                    Vector3 center;
                    center.x = (x + z * 0.5f - z / 2) * (HexMetrics.innerRadius * 2f);
                    center.y = 0.1f;
